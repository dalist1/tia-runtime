...
import {createBashTool, DEFAULT_MAX_LINES} from '@earendil-works/pi-coding-agent'
...
type Request = {id?: string; tool?: string; path?: string; offset?: number; limit?: number; contentFile?: string; templateFile?: string; oldTextFile?: string; newTextFile?: string; command?: string}

type UpdateStats = {updates: number; firstUpdateMs: number | null; observedBytes: number}
...
let tempNonce = 0
const tempDir = () => {
 const dir = join(tmpdir(), `pi-fast-daemon-${process.pid}-${++tempNonce}`)
...
function assertFileText(path: string, expected: string) {
 ...
 if (actual !== expected) {
  throw new Error(`write verification failed for ${path}: expected ${expected.length} chars/${Buffer.byteLength(expected, 'utf8')} bytes, got ${actual.length} chars/${Buffer.byteLength(actual, 'utf8')} bytes`)
 }
}
...
async function runBinaryWithInput(cmd: string, args: string[], input: string) {
 const proc = Bun.spawn([cmd, ...args], {cwd: ROOT_DIR, stdin: 'pipe', stdout: 'ignore', stderr: 'pipe'})
...
async function runBinary(cmd: string, args: string[]) {
 const proc = Bun.spawn([cmd, ...args], {cwd: ROOT_DIR, stdout: 'ignore', stderr: 'pipe'})
...
async function fastRead(pathArg: string, offset?: number, limit?: number, onUpdate?: OnUpdate) {
 ...
 const proc = Bun.spawn([FASTREAD_BIN, absolutePath, String(startLine), String(maxLines)], {cwd: ROOT_DIR, stdout: 'pipe', stderr: 'pipe'})
 const stderrPromise = new Response(proc.stderr).text()
...
async function fastBash(command: string) {
 ...
 await tool.execute('fast-bash', {command}, undefined, undefined)
}
...
async function handleRequest(request: Request) {
 ...
 try {
  if (tool === 'read') {
   ...
   output({id, ok: true, tool, mode, elapsedMs: performance.now() - startedAt, bytes, updates: stats.updates, firstUpdateMs: stats.firstUpdateMs, observedBytes: stats.observedBytes})
   return
  }
  ...
  if (tool === 'bash') {
   ...
   const command = request.command ?? `cat ${ROOT_DIR}/payloads/jsonl-5m.txt > /dev/null && cp ${ROOT_DIR}/payloads/jsonl-5m.txt ${copyPath} && rm ${copyPath}`
  ...
  output({id, ok: false, tool, mode, elapsedMs: performance.now() - startedAt, error: error instanceof Error ? error.message : String(error)})
 }
}
