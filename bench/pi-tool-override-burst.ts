...
let tempNonce = 0
const tempDir = () => {
 const dir = join(tmpdir(), `pi-fast-tool-${process.pid}-${++tempNonce}`)
...
async function runBinaryCapture(cmd: string, args: string[]) {
 const proc = Bun.spawn([cmd, ...args], {cwd: ROOT_DIR, stdout: 'pipe', stderr: 'pipe'})
...
async function runBinary(cmd: string, args: string[]) {
 const proc = Bun.spawn([cmd, ...args], {cwd: ROOT_DIR, stdout: 'ignore', stderr: 'ignore'})
