...
function mean(values: number[]) {
 return values.length > 0 ? values.reduce((a, b) => a + b, 0) / values.length : Number.POSITIVE_INFINITY
}
...
function isBaseline(command: string) {
 const lower = command.toLowerCase()
 return lower.startsWith('stock') || lower.includes('original') || lower.includes('baseline') || lower.includes('pi cli rpc')
}
...
function collect(resultDir: string) {
 ...
 for (const dir of readdirSync(resultDir)
  ...
  for (const file of readdirSync(roundDir).filter(name => name.endsWith('.json') && name !== 'meta.json')) {
...
function finalize(suites: Record<string, Record<string, AnyObj>>, baselines: Record<string, string>) {
 ...
 const topStrategies = Object.entries(strategyScores)
  ...
  .sort((a, b) => b.wins - a.wins || a.avg_relative_score_vs_baseline - b.avg_relative_score_vs_baseline || b.avg_speedup_vs_baseline - a.avg_speedup_vs_baseline || a.avg_cv - b.avg_cv)
  .slice(0, 3)
...
function writeMarkdown(resultDir: string, config: AnyObj, summary: AnyObj) {
 ...
 if (Object.keys(config).length > 0) {
  lines.push('## Config', '')
  for (const key of ['rounds', 'runs', 'warmup', 'tier', 'run_startup', 'zig']) if (key in config) lines.push(`- ${key}: \`${config[key]}\``)
  lines.push('')
 }
 ...
 for (const entry of Object.entries(summary.suites).sort()) {
  ...
  const item = entry[1]
  const w = item.winner
  lines.push(`| \`${suite}\` | ${w.command} | ${fmtMs(w.mean_ms)} ms | ${fmtSpeedup(w.speedup_vs_baseline)} | ${(w.cv * 100).toFixed(1)}% | ${(w.success_rate * 100).toFixed(0)}% |`)
 }
