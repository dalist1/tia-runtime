...
class DaemonClient {
 private child = spawn(`${ROOT_DIR}/bin/pi-tool-override-daemon`, [mode], {cwd: ROOT_DIR, stdio: ['pipe', 'pipe', 'pipe']})
 private buffer = ''
 private nextId = 1
...
async function runSpawnLoop() {
 ...
 return {transport, mode, tool, iterations, daemonReadyMs: null, firstResponseMs, elapsedMs: end - start, perIterationMs: (end - start) / iterations}
}
...
async function runDaemonLoop() {
 ...
 return {transport, mode, tool, iterations, daemonReadyMs, firstResponseMs, elapsedMs: end - start, perIterationMs: (end - start) / iterations}
}
