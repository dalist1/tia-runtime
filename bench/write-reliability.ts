...
async function withFileQueue<T>(path: string, task: () => Promise<T>) {
 const previous = fileQueues.get(path)
 let release = () => {}
 ...
 try {
  ...
  release()
  if (fileQueues.get(path) === current) fileQueues.delete(path)
 }
}
...
async function assertText(path: string, expected: string) {
 ...
 if (actual !== expected) {
  throw new Error(`write mismatch for ${path}: expected ${expected.length} chars/${Buffer.byteLength(expected, 'utf8')} bytes, got ${actual.length} chars/${Buffer.byteLength(actual, 'utf8')} bytes`)
 }
}
...
async function verifiedWrite(path: string, content: string) {
 return withFileQueue(path, async () => {
  ...
  const tmpPath = `${path}.tmp.${process.pid}.${++temporaryNonce}`
  try {
...
try {
 ...
 const concurrentContents = Array.from({length: 12}, (_, index) => `concurrent-${index}\n${'z'.repeat(index * 17)}\n`)
...
console.log(JSON.stringify({ok: true, iterations, cases: cases.length, writes, elapsedMs, perWriteMs: elapsedMs / writes}))
