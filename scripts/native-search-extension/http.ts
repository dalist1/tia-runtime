...
export async function fetchTextUrl(urlString: string, options: FetchTextOptions) {
  ...
  const accept =
    options.accept ??
    "text/markdown,text/plain;q=0.95,text/html;q=0.9,application/xhtml+xml;q=0.8,*/*;q=0.2";
  ...
  return withOriginRateLimit(url.origin, options.signal, async () => {
    ...
    try {
      ...
      const result: FetchTextResult = {
        url: normalized,
        finalUrl: normalizeHttpUrl(response.url || normalized),
      ...
      if (!response.ok && !options.allowHttpErrors)
        throw new Error(`HTTP ${response.status} fetching ${normalized}`);
...
async function withOriginRateLimit<T>(
  origin: string,
