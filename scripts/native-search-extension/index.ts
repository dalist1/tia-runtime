...
const nativeSearchSchema = {
 ...
 properties: {
  ...
  maxPages: {type: 'number', description: 'Maximum candidate pages/URLs to plan across all sites before fetch pre-ranking (default 12, hard max 50).'},
  fetchPages: {type: 'number', description: 'Maximum planned pages/URLs to actually fetch/extract after URL pre-ranking (default 5, hard max 50).'},
  adaptiveFetch: {type: 'boolean', description: 'Experimental: fetch additional planned URL batches when the first fetched batch has low score confidence. Default false; direct URL mode always fetches explicit URLs.'},
