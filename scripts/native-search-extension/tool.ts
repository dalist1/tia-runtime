...
const ZIG_SEARCH_BIN =
  process.env.TIA_NATIVE_SEARCH_ZIG_BIN ??
  new URL("../../fast-tools/native-search-zig", import.meta.url).pathname;

export async function runNativeSearchTool(
  ...
  const pagesPerSite = clampInteger(
    params.pagesPerSite,
    DEFAULT_PAGES_PER_SITE,
    1,
    HARD_MAX_PAGES_PER_SITE,
  );
  const contentChars = clampInteger(
    params.contentChars,
    DEFAULT_CONTENT_CHARS,
    1000,
    HARD_MAX_CONTENT_CHARS,
  ...
  const sites =
    strategy === "direct"
      ? seedSites(params.sites, queryUrls, envSites, maxSites)
      : autoDirectUrlMode
        ? queryUrls.slice(0, maxSites)
        : seedSites(params.sites, queryUrls, envSites, maxSites);
      ...
      content: [
        {
          type: "text",
          text:
            "Native search is intentionally site-bounded and does not call third-party search engines. " +
            "Provide one or more `sites`, include URLs in `query`, or set TIA_NATIVE_SEARCH_SEEDS.",
        },
      ],
      details: { query, resultCount: 0, reason: "missing_sites" },
    };
  }
  ...
  const discovered = directUrlMode
    ? sites.map((url) => ({ url, source: "direct URL", priority: 100 }))
    : discoveries.flatMap((discovery) => discovery.urls);
  const urls = planCandidateUrls(discovered, {
    strategy,
    maxPages: directUrlMode ? Math.min(maxPages, sites.length) : maxPages,
    perSiteCap: pagesPerSite,
  });

  if (!existsSync(ZIG_SEARCH_BIN)) {
    throw new Error(
      `native_search requires compiled Zig backend at ${ZIG_SEARCH_BIN}. Re-run install or bench/build-native-search-zig.sh.`,
    );
  }
  ...
  try {
    ...
    const proc = Bun.spawn(
      [
        ...
        String(options.contentChars),
        urlPath,
        String(originIntervalMs()),
        options.outputContent ? "1" : "0",
      ],
    ...
    if (exitCode !== 0)
      throw new Error(stderrText.trim() || `native-search-zig exited with code ${exitCode}`);
    return {
      content: [
        {
          type: "text",
          text: options.includePlan
            ? `${options.plan}\n\n${stdoutText.trimEnd()}`
            : stdoutText.trimEnd(),
        },
      ],
...
function planCandidateUrls(
  items: DiscoveredUrl[],
  options: { strategy: SearchStrategy; maxPages: number; perSiteCap: number },
) {
...
function compactDiscovered(items: DiscoveredUrl[]) {
  ...
  return Array.from(best.values()).sort(
    (a, b) =>
      ...
      originOf(a.url).localeCompare(originOf(b.url)) ||
...
function buildPlanText(input: {
  ...
  for (const item of input.urls)
    originCounts.set(originOf(item.url), (originCounts.get(originOf(item.url)) ?? 0) + 1);
  const lines = [
  ...
  const errors = input.discoveries.flatMap((discovery) =>
    discovery.errors.map((error) => `${discovery.site}: ${error}`),
  );
