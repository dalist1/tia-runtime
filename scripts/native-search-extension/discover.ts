...
export async function discoverSiteUrls(options: {
  site: string;
  pagesPerSite: number;
  timeoutMs: number;
  queryTerms?: string[];
  ...
  for (let index = 0; index < queue.length && index < MAX_DISCOVERY_DOCS_PER_SITE; index += 1) {
    ...
    try {
      const fetched = await fetchTextUrl(docUrl, {
      ...
      for (const url of parsed.urls) {
        ...
        found.push({
          url,
          source: labelFor(docUrl),
          priority: priorityFor(url, parsed.kind, options.queryTerms ?? []),
        });
      }
...
function compactDiscovery(
  ...
  for (const item of urls) {
    ...
    if (!current || item.priority > current.priority)
      best.set(normalized, { ...item, url: normalized });
  }
...
function parsePageLinks(text: string, baseUrl: string, contentType: string) {
  const urls =
    /html/i.test(contentType) || /<a\b/i.test(text)
      ? htmlLinks(text, baseUrl)
      : markdownLinks(text, baseUrl);
  urls.push(...extractUrls(text));
