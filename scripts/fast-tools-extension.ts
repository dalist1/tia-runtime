...
type TextToolUpdate = {content: Array<{type: 'text'; text: string}>; details?: any}
 ...
 const appendLine = (line: string) => {
  if (currentLine >= startLine) {
   if (outputLines >= maxLines) {
    ...
    return {content: [{type: 'text', text: `${output}\n\n[Showing lines ${startLine}-${endLine}. Use offset=${nextOffset} to continue.]`}], details: {truncation: {truncated: true, truncatedBy: 'lines', outputLines}}}
   }
   ...
   if (!agentSkill && outputBytes + nextBytes > DEFAULT_MAX_BYTES) {
    ...
    const endLine = startLine + outputLines - 1
    const nextOffset = endLine + 1
    return {content: [{type: 'text', text: `${output}\n\n[Showing lines ${startLine}-${endLine} (${formatSize(outputBytes)} limit). Use offset=${nextOffset} to continue.]`}], details: {truncation: {truncated: true, truncatedBy: 'bytes', outputLines}}}
   }
...
async function fastWrite(cwd: string, pathArg: string, content: string, signal?: AbortSignal) {
 ...
 return withFileMutationQueue(absolutePath, async () => {
  ...
  return {content: [{type: 'text', text: `Successfully wrote and verified ${bytes} bytes to ${pathArg}`}], details: {verified: true, bytes}}
 })
}
...
async function fastEdit(cwd: string, pathArg: string, edits: ReplacementEdit[], signal?: AbortSignal) {
 const absolutePath = resolvePath(cwd, pathArg)
 ...
 return withFileMutationQueue(absolutePath, async () => {
  ...
  if (existsSync(FASTEDIT_BIN) && edits.length === 1) {
   ...
   try {
    ...
    return {content: [{type: 'text', text: `Successfully replaced 1 block(s) in ${pathArg}.`}], details: undefined}
   } finally {
  ...
  const replacements = edits.map((edit, index) => {
   ...
   if (firstIndex === -1) {
    throw new Error(`Could not find edit ${index + 1} in ${pathArg}. The old text must match exactly including all whitespace and newlines.`)
   }

   const secondIndex = content.indexOf(edit.oldText, firstIndex + edit.oldText.length)
   if (secondIndex !== -1) {
    throw new Error(`Found multiple occurrences for edit ${index + 1} in ${pathArg}. The old text must be unique.`)
   }

   return {index, start: firstIndex, end: firstIndex + edit.oldText.length, newText: edit.newText}
  })
  ...
  ensureNotAborted(signal)
  return {content: [{type: 'text', text: `Successfully replaced ${edits.length} block(s) in ${pathArg}.`}], details: undefined}
 })
}
